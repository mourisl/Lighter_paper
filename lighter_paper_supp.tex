...
In that paper, the author further proposed \emph{pattern-blocked} Bllom filter, where the difference is that instead of updating the $h$ positions in the block separately, we can pre-compute a list of patterns where each pattern is a bit-mask that can update $h$ positions in a block with just a simple bit operation. When update, we find the pattern using hash function and update the corresponding position simultaneously. In Lighter, we used the 64-bit integers to form the mask. For example, if the $b=256$, then  the pattern is formed by 4 64-bit integers and we can update the corresponding positions with only 4 bit operations regardless of $h$. And we still use the FPR formula above to roughly estimate the FPR for the pattern-blocked bloom filter.
