...
\paragraph{Accuracy on simulated data.} We compared \tool v1.0.2's performance \myworries{(Li: ensure we make a release for the relevant version of the Lighter repo so that it's obvious what version people should use to recreate our results.  Think this should be v1.0.2.)} with Quake v0.3\cite{kelley2010quake}, Musket v1.1\cite{liu2013musket}, BLESS v0p17 \cite{heo2014bless}, and SOAPec v2.0.1 \cite{luo2012soapdenovo2} \myworries{(Li: is this version number right?)}.  We generated collection of reads simulated from the reference genome for the K12 strain of \ecoli (NC\_000913.2) using Mason v0.1.2 \cite{holtgrewe2010mason}.  
...
Both used a \kmer size is 23 \myworries{(both used this size, right?)}.
...
We also evaluated \tool's effect on alignment and assembly using a dataset from the GAGE project \cite{salzberg2012gage}.  The dataset consists of real 101 $\times$ 101 bp paired-end reads covering human chromosome 14 to $35\times$ average coverage ($\sim$36.5M reads).  We set the \kmer length to 19 for all error correctors for these experiments.
...
\section*{Tables} %1
\begin{table}[h!]
\caption{Accuracy measures for simulated data sets. Different error rate(\%) for each table for different coverages}
