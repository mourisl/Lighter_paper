...
\paragraph{Accuracy on simulated data.} We compared \tool v1.0.2's performance \myworries{(Li: ensure we make a release for the relevant version of the Lighter repo so that it's obvious what version people should use to recreate our results.  Think this should be v1.0.2. Answer: Done)} with Quake v0.3\cite{kelley2010quake}, Musket v1.1\cite{liu2013musket}, BLESS v0p17 \cite{heo2014bless}, and SOAPec v2.0.1 \cite{luo2012soapdenovo2} \myworries{(Li: is this version number right? Answer: Yes.)}.  We generated collection of reads simulated from the reference genome for the K12 strain of \ecoli (NC\_000913.2) using Mason v0.1.2 \cite{holtgrewe2010mason}.  
...
Both used a \kmer size is 23 \myworries{(both used this size, right? Right. But in Table 1, their reported kmer size is different.)}.
...
\subsubsection*{C.Elegans} % Here or Supp?
